...
const productIcons = [
  ...
  // X-Govt Tenders icon
  <span><svg key="icon7" width="34" height="34" viewBox="0 0 34 34" fill="none"><rect width="34" height="34" rx="8" fill="#f59e42" opacity="0.14"/><path d="M7 24h20M17 10v14" stroke="#f59e42" strokeWidth="2.5" strokeLinecap="round"/><circle cx="17" cy="17" r="5.5" stroke="#9333ea" strokeWidth="2.5"/></svg></span>
];
...
const items = [
  ...
  { name: 'X-Govt Tenders', desc: 'Get summarized, state-wise tenders for all of India, with monitoring, notifications, and lawyer connections. Premium required for full access.', price: 'Premium', badge: 'Premium', link: '/x-govt-tenders', iconIdx: 6 },
];

export default function Products() {
  const [hovered, setHovered] = useState(-1);
  return (
    <section className="section" style={{background: 'linear-gradient(135deg, #f8fafc 0%, #e0e7ef 100%)', minHeight: '100vh'}}>
      <div className="text-center mb-8">
        ...
        <p className="text-xl font-semibold text-gray-700 mb-2">Empowering you to connect, discover, and save—faster than ever.</p>
        <p className="mt-2 muted text-lg text-gray-500">Tooling and templates that help you ship faster and smarter.</p>
      </div>
      <div className="w-full flex justify-center mb-10">
        <svg height="48" width="100%" className="opacity-30"><ellipse cx="50%" cy="24" rx="48%" ry="18" fill="#e0e7ef" /></svg>
      </div>
      <div className="mt-8 grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 justify-center">
        {items.map((p, i) => {
          let cardGradient = '';
          ...
          else if (p.name === 'X-Govt Tenders') cardGradient = 'from-yellow-100/40 via-orange-400/10 to-indigo-100/10';
          else cardGradient = 'from-white/20 via-gray-100/10 to-white/10';
          return (
            <div
              key={i}
              className={`card relative group transition-all duration-300 ${hovered===i?' ring-2 ring-brand-500/40 scale-[1.03] shadow-glow':''} bg-gradient-to-br ${cardGradient} backdrop-blur-xl shadow-2xl rounded-3xl border border-gray-200/40 p-8 flex flex-col items-center text-center`}
              style={{ minHeight: 260, cursor: 'pointer', boxShadow: '0 8px 32px 0 rgba(60,60,90,0.08)' }}
              onMouseEnter={()=>setHovered(i)}
              onMouseLeave={()=>setHovered(-1)}
            >
              <div className="flex items-center gap-3 mb-3">
                <span className="drop-shadow-lg">{productIcons[p.iconIdx]}</span>
              ...
              <h3 className="text-xl font-semibold mt-1 drop-shadow bg-gradient-to-r from-fuchsia-400 via-orange-400 to-cyan-400 bg-clip-text text-transparent">
                {p.name}
              </h3>
              <p className="mt-2 muted text-base min-h-[44px] text-gray-700 font-light">{p.desc}</p>
              <div className="mt-6 flex items-center justify-between w-full">
                ...
                <Link href={p.link}>
                  <button className="btn rounded-xl px-5 py-2 bg-white/60 text-gray-800 font-semibold shadow hover:bg-white/80 transition-all duration-200" type="button">Learn more</button>
                </Link>
              </div>
            </div>
          );
      ...
      <div className="text-center mt-16">
        <svg height="48" width="100%" className="opacity-20 mb-4"><ellipse cx="50%" cy="24" rx="48%" ry="18" fill="#e0e7ef" /></svg>
        <p className="text-lg text-gray-400 font-light">Many more exciting products launching soon!</p>
      </div>
      <style jsx>{`
